...
- **Lo único que se desplaza en horizontal es la tabla; la página, nunca.** Ese ancho mínimo llegó a ensanchar la pantalla entera en el móvil: un item de grid o de flex tiene `min-width: auto` y **no se encoge por debajo del ancho mínimo de su contenido**, así que la columna crecía hasta los 46 rem y arrastraba a la página. Por eso la rejilla, sus dos columnas, el panel y los envoltorios de tabla y de bloque de código llevan **`min-w-0`**: es lo que mantiene el scroll encerrado donde toca. Si añades un contenedor nuevo en la cadena de resultados, ponle `min-w-0`. Como red —no como arreglo— `body` lleva `overflow-x: clip` en `app/globals.css`; es `clip` y no `hidden` porque `hidden` crearía un contenedor de scroll y rompería los tres `position: sticky` de la app.
